...
// ==============================================================
// 🎯 PhysicsWorld — 物理世界容器，所有物理对象的宿主
//
// 📌 作用：
//   PhysicsWorld 是 PhysicsCore2D 的顶级容器，
//   管理所有 PhysicsBody、PhysicsJoint、PhysicsShape 等对象，
//   控制模拟参数（重力、迭代数、睡眠策略等），
//   并提供世界级查询（碰撞检测、射线检测）和模拟控制。
//
// 🏗 架构 — 完整层级：
//   PhysicsWorld（世界）
//   ├── PhysicsBody（物理体）
//   │   ├── PhysicsShape（碰撞形状）→ Geometry（几何数据）
//   │   ├── PhysicsChain（链形碰撞体）
//   │   └── PhysicsJoint（约束连接）
//   ├── PhysicsSpace（空间加速结构，可选）
//   └── PhysicsQuery（碰撞查询）
//
// 💡 世界设置参数：
//   - 重力、弹跳阈值、接触频率/阻尼
//   - 模拟类型（离散/连续）、子步数、工作线程数
//   - 变换平面/写入模式/插值
//   - 睡眠策略、预热启动、接触过滤
//   - 绘制选项（调试可视化）
//
// ⚡ 世界级查询（批量 API）：
//   - TestOverlap*：布尔型重叠测试（AABB/Shape/Point/Geometry）
//   - Overlap*：结果型重叠查询
//   - CastRay/CastShape：射线/形状投射
//   - CastMover：移动器投射（用于角色控制器）
//
// 📌 事件系统：
//   - GetTriggerBeginEvents/GetTriggerEndEvents
//   - GetContactBeginEvents/GetContactEndEvents/GetContactHitEvents
//   - GetJointThresholdEvents
//   - GetBodyUpdateEvents
//
// 🎨 调试绘制：
//   丰富的 Draw* 方法集，支持绘制几何体、形状代理、AABB、查询结果等
// ==============================================================
