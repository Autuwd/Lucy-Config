...
// ==============================================================
// 🎯 PhysicsBody — 物理体，物理世界的核心实体
//
// 📌 作用：
//   PhysicsBody 是物理模拟的基本单元，对应物理世界中的一个刚体。
//   它拥有位置、速度、质量、形状等物理属性，
//   所有物理交互（力、碰撞、约束）都作用在 Body 上。
//
// 🏗 架构位置：World → Body → Shape
//   PhysicsWorld 包含多个 PhysicsBody
//   PhysicsBody 包含多个 PhysicsShape（碰撞形状）
//   PhysicsShape 引用几何体（Circle/Capsule/Polygon/Segment/Chain）
//
// 💡 关键能力：
//   - 创建/销毁（单个或批量）
//   - 位置/旋转/Transform 读写
...
//   - 物理属性（阻尼、重力缩放、摩擦、弹性）
//   - 睡眠/唤醒、约束、碰撞事件
//   - 批量操作优化（BatchVelocity/BatchForce/BatchTransform）
//
// ⚡ 批量操作：
//   使用 ReadOnlySpan<T> 实现零拷贝批量传输，
//   适用于需要高效处理大量物理体的场景。
//
// 📌 所有方法名遵循 PhysicsBody_{Method} 模式，
//   对应 C++ 端 PhysicsBody 类的相同接口。
// ==============================================================
