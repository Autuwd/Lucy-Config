...
// ==============================================================
// 🎯 PhysicsList<T> — 原生内存支持的动态数组（替代 NativeList）
//
// 📌 作用：
//   PhysicsList<T> 是一个轻量级的原生动态数组，
//   底层使用 UnsafeUtility.MallocTracked/FreeTracked 管理内存。
//   避免引入 Unity.Collections 的完整容器管线（安全句柄、AllocatorManager 等），
//   同时保持与 PhysicsBuffer 相同的分配原语。
//
// 🏗 架构位置：内部基础设施
//   被 PhysicsCore2D 内部用于临时数据收集和管理
//
// 💡 核心设计：
//   - StructLayout(LayoutKind.Sequential)：与 C++ 端二进制兼容
//   - 泛型 T : unmanaged：限定非托管类型
//   - 动态扩容（Grow）：容量翻倍策略（4 → 8 → 16...）
//   - 与 PhysicsBuffer 共享相同的 MallocTracked 分配接口
//
// 📌 关键方法：
//   - Add()：追加元素，自动扩容
//   - Resize()：调整长度
//   - SetCapacity()：预分配容量
//   - ToNativeArray()：移交所有权给 NativeArray（零拷贝）
//   - Sort()：原地排序（内省排序 Introsort）
//
// ⚡ ToNativeArray() 的特殊设计：
//   将自身缓冲区的所有权移交给新创建的 NativeArray，
//   调用后 PhysicsList 变为空状态（IsCreated = false），
//   由调用者负责 Dispose 返回的 NativeArray。
//
// 📌 排序算法：内省排序（Introsort）
//   与 NativeSortExtension.IntroSortStruct 完全相同的算法，
//   当递归深度超过 2*log2(n) 时降级为堆排序，避免快排最坏情况。
// ==============================================================
