...
// ==============================================================
// 🎯 PhysicsJoint — 约束系统，连接两个 PhysicsBody
//
// 📌 作用：
//   PhysicsJoint 是两个 PhysicsBody 之间的约束，
//   限制它们的相对运动。每个 Joint 连接 BodyA 和 BodyB。
//
// 🏗 架构位置：World → Body ←→ Joint → Body
//   Joint 引用两个 Body，将它们的运动耦合起来
//
// 🔗 Joint 类型：
//   - DistanceJoint：距离约束（弹簧/马达/限位）
//   - RelativeJoint：相对运动约束（速度/力控制）
//   - SliderJoint：滑动约束（沿轴平移）
//   - HingeJoint：铰链约束（绕点旋转）
//   - FixedJoint：固定约束（刚体连接）
//   - WheelJoint：车轮约束（弹簧悬挂 + 马达）
//   - IgnoreJoint：忽略碰撞约束
//
// 💡 公共能力（所有 Joint 共享）：
//   - 锚点（LocalAnchorA/B）：连接点在本地坐标中的位置
//   - 力/扭矩阈值：约束断裂检测
//   - 调谐频率/阻尼：软约束（弹簧行为）
//   - CollideConnected：约束物体间是否碰撞
//   - 世界绘制/所有权/用户数据
//
// 📌 每个 Joint 类型单独读写 Definition，
//   Definition 包含该类型的所有配置参数。
// ==============================================================
