...
// ==============================================================
// 🎯 PhysicsMiscellaneous — 杂项工具集
//
// 📌 作用：
//   汇集辅助数学类型和全局引擎设置的原生绑定。
//   包括 PhysicsTransform、PhysicsRotate、PhysicsAABB、
//   PhysicsPlane、PhysicsMath、PhysicsHandle 等基础设施。
//
// 🏗 架构位置：基础设施层，被所有其他组件引用
//
// 📐 工具类型：
//   - PhysicsTransform：2D 变换（位置 + 旋转）
//   - PhysicsRotate：2D 旋转（角度制，支持 lerp/积分/向量旋转）
//   - PhysicsAABB：轴对齐包围盒（重叠/联合/包含/射线检测）
//   - PhysicsPlane：平面（点到平面距离）
//   - PhysicsMath：数学常量（PI/TAU）和辅助函数（角度转换/弹簧阻尼）
//   - PhysicsHandle：引擎对象句柄（创建/销毁/有效性检查）
//
// 🌐 全局设置（#region Globals）：
//   - 项目设置读写（PhysicsCoreSettings）
//   - 默认世界、世界容量、安全锁
//   - 物理图层名称、长度单位
//   - 变换模式、渲染模式
//   - 接触过滤模式
