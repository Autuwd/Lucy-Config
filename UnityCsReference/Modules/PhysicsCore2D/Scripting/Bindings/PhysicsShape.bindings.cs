...
// ==============================================================
// 🎯 PhysicsShape — 碰撞形状，物理体的碰撞表面
//
// 📌 作用：
//   PhysicsShape 是实际参与碰撞检测的单元，
//   挂载在 PhysicsBody 上，包含一种几何体和表面材质属性。
//   一个 PhysicsBody 可以有多个 PhysicsShape。
//
// 🏗 架构位置：World → Body → Shape → Geometry
//   PhysicsShape 连接 PhysicsBody 和几何体数据
//
// 💡 关键能力：
//   - 创建 5 种形状：Circle/Capsule/Polygon/Segment/ChainSegment
//   - 表面材质：摩擦系数、弹性系数及混合模式
//   - 触发器（IsTrigger）：只检测不产生物理反馈
//   - 接触过滤（ContactFilter）：按类别/组过滤碰撞
//   - 事件回调：接触事件、触发器事件、命中事件
//   - 碰撞检测：OverlapPoint / ClosestPoint / CastRay / CastShape
//   - 几何体读写：Get/Set 每种几何体类型
//
// 📌 表面材质属性：
//   - Friction/Bounciness + MixingMode（如何与对方材质混合）
//   - FrictionPriority/BouncinessPriority
//   - RollingResistance（滚动阻力）
//   - TangentSpeed（表面切线速度，用于传送带效果）
//
// ⚡ 批量操作：
//   CreateShapeBatch() 支持在一次调用中创建多个形状
//   DestroyBatch() 支持批量销毁
// ==============================================================
