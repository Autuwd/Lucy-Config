...
// ==============================================================
// 🎯 PhysicsGeometry — 几何体类型集，定义碰撞形状的底层数据
//
// 📌 作用：
//   提供 5 种几何体类型的创建、验证、计算和碰撞检测函数。
//   几何体是 PhysicsShape 的数据载体，描述形状的"数学"表示。
//
// 🏗 架构位置：Shape → Geometry (数据层)
//   每种 PhysicsShape 包含一种几何体类型
//
// 📐 几何体类型：
//   - CircleGeometry：圆形（中心 + 半径）
//   - CapsuleGeometry：胶囊形（两个圆心 + 半径）
//   - PolygonGeometry：多边形（顶点列表 + 半径）
//   - SegmentGeometry：线段（两个端点，无体积）
//   - ChainSegmentGeometry：链段（链形碰撞体的一个段）
//
// 💡 每类几何体共同能力：
//   - IsValid()：验证几何体是否合法
//   - CalculateAABB()：计算包围盒
//   - CalculateMass()：计算质量配置（Circle/Capsule/Polygon）
//   - OverlapPoint()：点重叠测试
//   - ClosestPoint()：最近点查询
//   - CastRay()：射线检测
//   - CastShape()：形状投射
//
// ⚡ PolygonGeometry 额外能力：
//   - CreateBox()：快速创建盒子多边形
//   - CreatePolygons()：从顶点集生成多个多边形（支持 Delaunay）
//   - Transform/InverseTransform：坐标变换
//   - Validate()：验证并修复多边形
// ==============================================================
...
namespace Unity.U2D.Physics
{
    static partial class Scripting2D
    {
        ...
        [NativeMethod(Name = "PolygonGeometry::TransformSpan_WithPhysicsTransform", IsThreadSafe = true)] extern internal static void PolygonGeometry_TransformSpan_WithPhysicsTransform(Span<PolygonGeometry> geometries, PhysicsTransform transform);
        [NativeMethod(Name = "PolygonGeometry::InverseTransformSpan_WithPhysicsTransform", IsThreadSafe = true)] extern internal static void PolygonGeometry_InverseTransformSpan_WithPhysicsTransform(Span<PolygonGeometry> geometries, PhysicsTransform transform);
        [NativeMethod(Name = "PolygonGeometry::TransformSpan_WithMatrix", IsThreadSafe = true)] extern internal static void PolygonGeometry_TransformSpan_WithMatrix(Span<PolygonGeometry> geometries, Matrix4x4 transform, bool scaleRadius);
        [NativeMethod(Name = "PolygonGeometry::InverseTransformSpan_WithMatrix", IsThreadSafe = true)] extern internal static void PolygonGeometry_InverseTransformSpan_WithMatrix(Span<PolygonGeometry> geometries, Matrix4x4 transform, bool scaleRadius);
