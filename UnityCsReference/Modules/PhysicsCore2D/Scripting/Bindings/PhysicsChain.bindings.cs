...
// ==============================================================
// 🎯 PhysicsChain — 链形碰撞体，由多个线段组成的连续碰撞表面
//
// 📌 作用：
//   PhysicsChain 是一系列相连的线段（Segment）组成的碰撞体，
//   用于表示地形、墙壁、轨道等复杂轮廓。
//   每个链段（ChainSegment）是一个独立的 PhysicsShape，
//   但它们共享同一个 PhysicsBody。
//
// 🏗 架构位置：Body → Chain → ChainSegment(Shape)
//   PhysicsBody 挂载 PhysicsChain
//   PhysicsChain 由多个 ChainSegmentGeometry 组成
//   每个 ChainSegment 可单独查询（射线检测、最近点等）
//
// 💡 关键能力：
//   - 创建时指定顶点数组，可设为循环或非循环
...
//
// 🔗 关联类型：
//   - ChainSegmentGeometry：单个段的几何体
//   - ChainGeometry：完整的链条几何数据
// ==============================================================
