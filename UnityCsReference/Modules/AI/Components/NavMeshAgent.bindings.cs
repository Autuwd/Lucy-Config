...
// ==============================================================
// 🎯 NavMeshAgent — 导航 Agent 组件
//
// 📌 作用：
//   NavMeshAgent 是 Unity 中控制角色沿 NavMesh 自动寻路的核心组件。
//   它负责：寻路请求、路径跟随、障碍避让、OffMeshLink 穿越。
//
// 🔑 Agent Steering（转向控制）：
//   SetDestination(target) → 请求 A* 寻路计算
//   → pathPending = true（等待路径就绪）
//   → 路径就绪后，Agent 沿路径拐点移动
//   → 每帧根据 speed/angularSpeed/acceleration 更新位置和旋转
//   → remainingDistance <= stoppingDistance 时停止
//
// 💡 路径状态（pathStatus）：
//   PathComplete:  路径到达目的地 ✅
//   PathPartial:   无法完全到达，返回最近点 ⚠️
//   PathInvalid:   路径不可用 ❌（起点不在 NavMesh 上等）
//
// 🔄 Auto-Repath 机制：
//   autoRepath=true 时，当前路径失效后自动重新计算路径。
//   适用于动态障碍物环境中需要持续追踪目标。
//
// ⚡ 性能提示：
//   - 大量 Agent 时降低 obstacleAvoidanceType
//   - 使用 isStopped 暂停 Agent 比 Destroy 更高效
//   - Warp() 直接设置位置，避免寻路开销
// ==============================================================
...
{
    // ==============================================================
    // 🎯 ObstacleAvoidanceType — 障碍避让质量等级
    //
    // 📌 控制 Agent 的障碍避让精度与性能的权衡：
    //   NoObstacleAvoidance:  禁用避让（最低开销）
    //   Low/Med/Good/High:    从低到高递增的精度
    //
    // ⚡ 高精度 = 更高性能开销
    //   大量 Agent（10+）建议使用 Med 或 Low
    // ==============================================================
    ...
    //
    // 🎯 路径控制：
    //   - SetDestination / destination:  设置/获取目标位置
    //   - stoppingDistance:              停止距离
    //   - path / pathStatus:             当前路径和状态
    //   - remainingDistance:             到目标剩余距离
    //   - isPathStale / hasPath / pathPending: 路径状态查询
    //
    // 🚶 运动控制：
    //   - speed / angularSpeed / acceleration: 移动参数
    //   - velocity / desiredVelocity:    当前/期望速度
    //   - steeringTarget:                当前转向目标（下一个拐点）
    //   - updatePosition / updateRotation: 是否自动更新 Transform
    //   - autoBraking:                   是否自动减速
    //
    // 🔗 OffMeshLink：
    //   - isOnOffMeshLink:             是否在链接上
    //   - currentOffMeshLinkData:      当前链接详情
    //   - CompleteOffMeshLink:         手动完成链接穿越
    //   - autoTraverseOffMeshLink:     是否自动穿越
    //
    // 🛡 障碍避让：
    //   - radius / height:              碰撞体尺寸
    //   - obstacleAvoidanceType:        避让质量等级
    //   - avoidancePriority:            避让优先级
    //
    // ⚡ Warp() — 直接传送 Agent 到指定位置，
    //   不经过寻路计算，适用于传送门等场景。
    // ==============================================================
    [MovedFrom("UnityEngine")]
