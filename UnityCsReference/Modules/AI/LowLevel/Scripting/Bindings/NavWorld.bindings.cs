...
// ==============================================================
// 🎯 NavWorld — 低级 NavMesh 寻路世界
//
// 📌 作用：
//   基于 Detour 库的低级 NavMesh 查询 API。
//   提供 A* 寻路、NavMesh 采样、光线投射、位置移动等操作。
//   是 NavMeshAgent 等高层组件的底层实现基础。
//
// 🔑 核心 API：
//
// 📍 位置映射：
//   - MapLocation():    世界坐标 → NavLocation（位置+节点对）
//   - CreateLocation(): NavNode + position → NavLocation
//   - MoveLocation():   沿 NavMesh 移动位置到目标
//
// 📐 寻路（三段式 A*）：
//   - BeginFindPath():   初始化 A* 搜索（起点 + 终点）
//   - ContinueFindPath(): 继续搜索（可分批执行）
//   - EndFindPath():     完成搜索
//   - GetResultFromFindPath(): 获取路径节点数组
//
// 🔍 查询：
//   - Raycast():         沿 NavMesh 表面射线检测
//   - IsValid():         检查节点/位置是否有效
//   - GetPortalPoints(): 获取两个相邻节点的"门"点（用于路径平滑）
//   - GetEdgesAndNeighbors(): 获取节点的边和邻居
//   - GetNodeType():     查询节点类型
//   - GetAreaIndexForNode(): 查询节点所属区域
//
// 💡 与 NavMesh 高层 API 的关系：
//   NavMesh.SamplePosition / Raycast / CalculatePath
//   内部都是通过 NavWorld 的底层 API 实现的。
//   NavWorld 直接暴露底层查询，适合自定义寻路逻辑。
//
// ⚡ NativeContainer 设计：
//   支持在 Unity Job System 中使用，
//   通过 AtomicSafetyHandle 保证线程安全。
// ==============================================================
...
// 🎯 NavWorld — 低级寻路世界
//
// 📌 封装对底层 Native NavMesh 的引用
//   （m_NavMeshPtr + m_ImmutableQuery + m_UniqueId）。
//
// 💡 通过 GetDefaultWorld() 获取默认世界。
//   Dispose() 仅清理 C# 安全句柄，不销毁 C++ 的 NavMesh。
// ==============================================================
[NativeContainer]
