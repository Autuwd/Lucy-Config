...
{
    // ================================================================
    // VFXGraphicsStateCollectionBindings —— VFX 图形状态收集器
    //
    // 🎯 将 VFX 的图形状态（渲染状态/Shader Keyword/Pass 配置）
    //    预编译到 GraphicsStateCollection 中，加速运行时渲染设置
    // 💡 用途：
    //   - 为 GraphicsStateCollection 添加 VFX 所需的完整图形状态
    //   - 自动处理 INSTANCING_ON 关键字的有无两种变体
    //   - 支持从参考状态（refState）创建关联状态
    // 📌 使用场景：
    //   - 自定义渲染管线的 VFX 状态预编译
    //   - 减少运行时 VFX 渲染状态的构建开销
    // ================================================================
