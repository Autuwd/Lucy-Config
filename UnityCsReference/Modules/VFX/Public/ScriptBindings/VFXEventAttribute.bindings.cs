...
{
    // ================================================================
    // VFXEventAttribute —— VFX 事件属性数据包
    //
    // 🎯 附加在 VFX 事件上的参数集合，用于向 VFX Graph 传递自定义数据
    // 💡 事件系统：
    //   - VisualEffect.SendEvent() 可附带 VFXEventAttribute
    //   - 事件属性在 VFX Graph 中通过"事件属性"节点读取
    // 📌 支持 exposed 属性的读写：bool/int/uint/float/Vector2~4/Matrix4x4
    // ⚡ 生命周期：
    //   - 通过 VisualEffect.CreateVFXEventAttribute() 创建
    //   - 继承 VisualEffectAsset 中定义的 exposed 属性布局
    //   - 实现 IDisposable 释放非托管内存
    // ================================================================
