...
{
    // ================================================================
    // VFXExposedProperty —— VFX 暴露属性描述结构
    //
    // 🎯 记录 VisualEffectAsset 中暴露给外部的属性名和类型
    // 💡 通过 GetExposedProperties() 获取，用于动态绑定参数
    // ================================================================
    [UsedByNativeCode]
    ...
    // ================================================================
    // VisualEffectObject —— VFX 资源基类
    //
    // 🎯 VisualEffectAsset 和 VisualEffectSubAsset 的抽象基类
    // 💡 继承自 Object，是所有 VFX 资源的公共父类型
    // ================================================================
    [UsedByNativeCode]
    ...
    // ================================================================
    // VisualEffectAsset —— VFX 粒子效果资产
    //
    // 🎯 定义 VFX Graph 编译后的完整粒子效果资源
    // 📌 包含 PlayEventID / StopEventID 等标准事件 ID
    // 💡 通过 GetExposedProperties() 获取可脚本控制的参数列表
    // ⚡ 支持 GPU Instancing 模式配置 (instancingMode)
    // ================================================================
    [UsedByNativeCode]
    [NativeHeader("Modules/VFX/Public/VisualEffectAsset.h")]
    [NativeHeader("VFXScriptingClasses.h")]
    ...
    // ================================================================
    // VFXOutputEventArgs —— VFX 输出事件参数
    //
    // 🎯 当 VisualEffect 触发 outputEventReceived 时传递的事件数据
    // 💡 包含事件 nameId 和关联的 VFXEventAttribute
    // ================================================================
    ...
    // ================================================================
    // VisualEffect —— VFX Graph 粒子系统核心组件
    //
    // 🎯 挂载在 GameObject 上驱动 VFX 粒子效果的核心运行时组件
    // 💡 事件系统：
    //   - SendEvent() / Play() / Stop()：触发 VFX Graph 中的事件
    //   - initialEventID：初始化时自动发送的事件
    // 📌 参数系统（通过 nameID 或 string 名称访问）：
    //   - HasXXX() / GetXXX() / SetXXX()：检测/读写暴露的参数
    //   - 支持 bool/int/uint/float/Vector2/3/4/Matrix4x4/曲线/渐变等
    // ⚡ 运行时控制：
    //   - pause / playRate：暂停与播放速率
    //   - Simulate()：手动模拟 N 帧
    //   - Reinit()：重置并重新播放
    //   - AdvanceOneFrame()：步进一帧
    // 🎯 GPU 粒子架构：
    //   - VFX Graph 使用 Compute Shader 在 GPU 上更新粒子
    //   - CPU 只负责 Spawner 逻辑和事件调度
    //   - 粒子数据（位置/速度/颜色）完全存储在 GPU Buffer 中
    //   - 支持 GPU Instancing 批量渲染相同 Asset 的实例
    // ================================================================
    ...
    // ================================================================
    // VFXRenderer —— VFX 专用渲染器组件
    //
    // 🎯 VisualEffect 自动关联的内部渲染器，负责提交 GPU 绘制命令
    // 💡 继承自 Renderer，但禁用材质拖拽（RejectDragAndDropMaterial）
    // 📌 由 VisualEffect 组件自动管理，无需手动创建
    // ================================================================
    [RequiredByNativeCode]
    ...
    // ================================================================
    // VFXParticleSystemInfo —— VFX 粒子系统运行时信息
    //
    // 🎯 通过 GetParticleSystemInfo() 获取的粒子系统快照数据
    // 💡 包含存活粒子数/容量/休眠状态/包围盒等调试信息
    // ================================================================
    [UsedByNativeCode]
