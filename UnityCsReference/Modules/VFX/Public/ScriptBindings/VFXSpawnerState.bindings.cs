...
{
    // ================================================================
    // VFXSpawnerLoopState —— Spawner 循环状态
    //
    // 🎯 跟踪 Spawner 的循环阶段：延迟前 / 循环中 / 延迟后 / 结束
    // 💡 配合 delayBeforeLoop / loopDuration / delayAfterLoop 使用
    // ================================================================
    ...
    // ================================================================
    // VFXSpawnerState —— Spawner 运行时状态
    //
    // 🎯 在 VFXSpawnerCallbacks 中读写生成器当前帧的完整状态
    // 💡 核心字段：
    //   - spawnCount：【关键】设置此值控制本次发射多少粒子
    //   - deltaTime / totalTime：时间参数（受 playRate 影响）
    //   - loopState / loopIndex / loopCount：循环控制
    //   - delayBeforeLoop / loopDuration / delayAfterLoop：延迟参数
    // 📌 通过 vfxEventAttribute 获取触发事件的附加属性
    // ⚡ 每个 Spawner 每帧创建一个新的 VFXSpawnerState 包装对象
    // ================================================================
