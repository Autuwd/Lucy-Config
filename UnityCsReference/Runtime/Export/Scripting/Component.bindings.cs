...
// ==============================================================
// 🎯 Component — Unity 组件的基类
//
// 📌 作用：
//   所有可以挂载到 GameObject 上的组件都继承自此类。
//   Component 提供了获取自身和关联 GameObject 上其他组件的能力。
//
// 🏗 架构位置：
//   Object → Component → Behaviour → MonoBehaviour
//            Component → Transform / Renderer / Collider / ...
//
// 🔑 关键能力：
//   - gameObject: 获取所属的 GameObject
//   - transform: 获取所属 GameObject 的 Transform
//   - GetComponent<T>(): 获取同 GameObject 上的其他组件
//   - SendMessage/BroadcastMessage: 组件间消息传递
//
// 💡 Component 是一个"视图"：
//   Component 本身并不"拥有"GameObject，
//   而是通过 gameObject 属性获取对容器的引用。
//   这就是为什么你可以写：
//     GetComponent<Rigidbody>().AddForce() ← 同对象上的其他组件
// ==============================================================
...
namespace UnityEngine
{
    ...
    //
    // 🔑 核心设计原则：
    //   Component 本质上是一个"装饰器"，
    //   它通过挂载到 GameObject 上来为对象添加功能。
    //   GetComponent<T>() 是 Unity 组件模式的核心，
    //   它允许组件之间互相发现和交互。
    //
    // 💡 组件的生命周期由所属 GameObject 控制：
    //   - GameObject.SetActive(false) → 所有 Component.OnDisable()
    //   - GameObject.Destroy() → 所有 Component.OnDestroy()
    // ==============================================================
