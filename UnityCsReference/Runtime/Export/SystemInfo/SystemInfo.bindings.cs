...
// ==============================================================
// 🎯 SystemInfo — 系统与 GPU 能力查询类
//
// 📌 作用：
//   提供对运行设备硬件信息的只读查询，包括 GPU、CPU、内存、
//   操作系统、以及各种渲染特性的支持情况。
//
// 🏗 核心查询类别：
//
//   【GPU 基本信息】
//   - graphicsDeviceName：GPU 名称（如 "NVIDIA GeForce RTX 3080"）
//   - graphicsDeviceVendor：GPU 厂商（如 "NVIDIA"）
//   - graphicsDeviceType：图形 API 类型（Direct3D12、Vulkan、Metal 等）
//   - graphicsDeviceVersion：图形 API 版本字符串
//   - graphicsMemorySize：显存大小（MB）
//   - graphicsShaderLevel：Shader 级别（如 50 = Shader Model 5.0）
//
//   【GPU 渲染能力查询】
//   - supportsInstancing：GPU 实例化渲染
//   - supportsComputeShaders：计算着色器
//   - supportsGeometryShaders：几何着色器
//   - supportsTessellationShaders：曲面细分着色器
//   - supportsShadows：阴影支持
//   - supportsMotionVectors：运动向量（用于 TAA、Motion Blur）
//   - supportsRayTracing：光线追踪支持
//   - supportedRenderTargetCount：多渲染目标（MRT）数量
//   - supportsTextureWrapModes：纹理寻址模式支持
//
//   【纹理格式支持】
//   - SupportsTextureFormat：运行时查询指定纹理格式是否受支持
//   - IsFormatSupported：查询 GraphicsFormat 的支持情况
//   - GetCompatibleFormat：获取兼容的替代格式
//
//   【CPU 与系统信息】
//   - processorType / processorCount / processorFrequency
//   - systemMemorySize：系统内存（MB）
//   - operatingSystem：操作系统名称
//   - deviceType：设备类型（Desktop/Console/Handheld）
//   - supportsVibration / supportsGyroscope 等传感器能力
//
// 💡 理解关键：
//   SystemInfo 的所有属性都是只读的，反映运行时的实际硬件能力。
//   开发者常用它来动态调整画质设置、选择合适的纹理格式、
//   降级功能特性（Feature Downgrading）。
//
// ⚡ 使用场景：
//   - 根据 GPU 能力自动选择画质等级
//   - 根据支持情况启用/禁用特定渲染特性
//   - 崩溃报告中收集设备信息
//
// 📍 对应 C++ 头文件：Runtime/Misc/SystemInfo.h
// ==============================================================
...
{
    // ==============================================================
    // BatteryStatus — 电池状态枚举
    // 🎯 移动设备电池状态：Unknown / Charging / Discharging / NotCharging / Full
    // ==============================================================
    ...
    //
    // 🔑 关键 GPU 属性：
    //   - graphicsDeviceName / graphicsDeviceVendor：GPU 名称和厂商
    //   - graphicsDeviceType：图形 API 类型（D3D11/D3D12/Vulkan/Metal/...）
    //   - graphicsMemorySize：显存大小
    //   - graphicsShaderLevel：Shader 版本级别（45=SM4.5, 50=SM5.0）
    //   - graphicsMultiThreaded：是否支持多线程渲染
    //   - renderingThreadingMode：渲染线程模式
    //
    // 🔑 关键渲染能力属性：
    //   - supportsInstancing：GPU 实例化
    //   - supportsComputeShaders：计算着色器
    //   - supportsGeometryShaders：几何着色器
    //   - supportsTessellationShaders：曲面细分
    //   - supportsShadows：阴影
    //   - supportsMotionVectors：运动向量（TAA/Motion Blur 需要）
    //   - supportsRayTracing：光线追踪
    //   - supportedRenderTargetCount：MRT 数量
    //   - hasTiledGPU：是否为 TBR 架构（移动端常见）
    //
    // 💡 实际使用示例：
    //   if (!SystemInfo.supportsComputeShaders) { /* 降级方案 */ }
    //   if (SystemInfo.graphicsMemorySize < 2048) { /* 低画质 */ }
    // ==============================================================
