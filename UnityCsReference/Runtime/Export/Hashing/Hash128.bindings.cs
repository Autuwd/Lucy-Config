...
//
// 📌 作用：
//   生成和比较 128 位哈希，用于 AssetBundle 版本标识、数据完整性校验
//
// 💡 关键概念：
//   - Compute(): 从 string / NativeArray / T[] / List<T> / ref T 计算哈希
//   - Append(): 在已有哈希基础上增量添加数据
//   - Parse(): 从字符串解析哈希值（"00000000000000000000000000000000"）
//   - isValid: 检查哈希是否非零
//
// ⚠️ 关键要求：
//   - Compute<T> 要求 T 是 blittable（非 blittable 类型会抛异常）
//   - 内部使用 ShortHash4 对小值（int/float）做纯托管路径加速
//
// 💡 用于 Caching.IsVersionCached / AssetBundle 版本标识
// ==============================================================
