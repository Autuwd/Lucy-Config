...
// ==============================================================
// 🎯 Debug — Unity 调试/日志系统的核心类
//
// 📌 作用：
//   提供日志输出、断言、调试绘制等开发期工具
//
// 💡 日志层级：
//   - Debug.Log()      → LogType.Log（白色）
//   - Debug.LogWarning() → LogType.Warning（黄色）
//   - Debug.LogError()   → LogType.Error（红色）
//   - LogException()     → LogType.Exception
//   - Assert()           → LogType.Assert（仅在 #UNITY_ASSERTIONS 下编译）
//
// 🎯 关键提醒：
//   - Debug 是 partial class，日志通过 ILogger 接口可替换
//   - DrawLine/DrawRay 仅编辑器中可见（Gizmos 同类）
//   - Break() 暂停编辑器，DebugBreak() 触发调试器断点
//
// ⚠️ isDebugBuild 只在 Development Build 下返回 true
// ==============================================================
