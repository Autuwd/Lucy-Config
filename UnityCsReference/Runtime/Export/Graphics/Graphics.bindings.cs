...
// ==============================================================
// 🎯 Graphics — Unity 核心图形渲染 API
//
// 📌 作用：
//   Graphics 是 Unity 提供的底层图形 API 入口类，提供即时模式（Immediate Mode）
//   的渲染调用。它是 Camera 自动渲染循环之外的"手动渲染"工具。
//
// 🏗 核心概念：
//   - DrawMesh / DrawMeshInstanced / DrawMeshInstancedIndirect：
//     三种绘制网格的方式，性能从低到高，灵活度从高到低
//   - Blit：纹理拷贝/后处理核心操作，将源纹理通过材质渲染到目标
//   - RenderMesh / RenderPrimitives：新版 RenderParams 渲染 API（2021+）
//   - CopyTexture：GPU 端纹理拷贝（比 Blit 更高效）
//   - ExecuteCommandBuffer：执行 CommandBuffer 中的渲染命令
//
// 💡 理解关键：
//   Graphics 类的调用都发生在当前活跃的 Camera 渲染循环中，
//   或者通过 CommandBuffer 在自定义渲染流程中使用。
//   Graphics.DrawMesh 等调用不会被 Camera 自动管理，需要手动管理生命周期。
//
// ⚡ 性能分层：
//   Graphics.DrawMeshInstancedIndirect > Graphics.DrawMeshInstanced > Graphics.DrawMesh
//   批量渲染优先使用 GPU Instancing，避免逐个 DrawMesh 调用。
//
// 📍 对应 C++ 头文件：Runtime/Graphics/GraphicsScriptBindings.h
// ==============================================================
...
{
    // ==============================================================
    // LightProbesQuery — Job System 友好的光照探针查询结构体
    //
    // 🎯 用途：在多线程 Job 中高效查询插值后的光照探针和遮挡探针数据
    // 💡 与 LightProbes.GetInterpolatedProbe 不同，这个结构体可在线程安全的 Job 中使用
    // ⚡ 通过 NativeArray 批量计算多个位置的光照探针，适合大规模 GPU 粒子/植被系统
    // ==============================================================
    [NativeContainer]
    ...
    // ==============================================================
    // EnabledOrientation — 屏幕自动旋转方向位掩码
    // 🎯 用于控制移动设备在哪些方向上自动旋转屏幕
    // ==============================================================
    ...
    // ==============================================================
    // FullScreenMode — 全屏模式枚举
    //
    // 🎯 定义窗口/全屏的显示模式：
    //   - ExclusiveFullScreen：独占全屏，直接接管显示器输出（最低延迟）
    //   - FullScreenWindow：全屏窗口化，无边框全屏（推荐，切换 alt-tab 更流畅）
    //   - MaximizedWindow：最大化窗口
    //   - Windowed：普通窗口模式
    //
    // 💡 推荐使用 FullScreenWindow，因为它支持更好的多任务切换体验。
    // ==============================================================
    ...
    // ==============================================================
    // RefreshRate — 刷新率结构体（分子/分母分数表示）
    //
    // 🎯 用于精确表示显示器刷新率（如 60Hz = 60/1, 144Hz = 144/1, 59.94Hz = 5994/100）
    // 💡 使用分数形式避免浮点精度问题，确保刷新率比较的准确性
    // ==============================================================
    ...
    // ==============================================================
    // DisplayInfo — 显示器详细信息结构体
    //
    // 🎯 包含分辨率、刷新率、DPI、工作区域等显示器完整信息
    // ⚡ 用于多显示器环境下获取每个显示器的物理参数
    // ==============================================================
    [UsedByNativeCode]
    ...
    // ==============================================================
    // Screen（EditorScreen）— 屏幕管理内部实现
    //
    // 🎯 管理屏幕分辨率、全屏模式、MSAA、DPI 等显示参数
    // 📌 Screen 类是公开 API，EditorScreen 是内部实现（通过 ShimManager 桥接）
    // 💡 Screen.SetResolution 是游戏中最常用的屏幕管理 API
    // ==============================================================
...
{
    // ==============================================================
    // MemorylessMode — 无内存深度缓冲模式
    //
    // 🎯 控制深度缓冲是否使用"无内存"模式（移动端优化）
    //   - Unused：不使用无内存模式
    //   - Forced：强制使用（节省内存但可能降低性能）
    //   - Automatic：根据平台自动决定
    // ==============================================================
...
{
    // ==============================================================
    // ComputeBufferMode — Compute Buffer 访问模式
    //
    // 🎯 控制 ComputeBuffer 在 GPU 端的访问策略
    //   - Immutable：创建后不可修改（适合静态数据）
    //   - Dynamic：频繁更新（CPU→GPU 每帧传输）
    //   - SubUpdates：部分更新（只更新 buffer 的一小块区域）
    // ==============================================================
...
{
    // ==============================================================
    // Graphics — 底层图形渲染 API 核心类
    //
    // 📌 作用：
    //   Graphics 提供不依赖 Camera 自动渲染循环的手动渲染能力。
    //   常用于：后处理效果、自定义渲染、程序化绘制、Compute Shader 输出等。
    //
    // 🎯 关键方法分类：
    //
    //   【网格绘制】
    //   - DrawMesh：绘制单个网格（立即执行）
    //   - DrawMeshInstanced：GPU Instancing 批量绘制（最多 1023 个实例）
    //   - DrawMeshInstancedProcedural：程序化绘制（通过 GPU 直接指定实例数）
    //   - DrawMeshInstancedIndirect：间接绘制（GPU 自主决定绘制数量，最大灵活性）
    //   - RenderMesh：新版 RenderParams 绘制 API
    //   - RenderMeshInstanced：新版批量绘制
    //
    //   【纹理操作】
    //   - Blit：将源纹理通过材质渲染到目标（后处理核心）
    //   - CopyTexture：GPU 端纹理拷贝（同格式快速拷贝）
    //   - ConvertTexture：带格式转换的纹理拷贝
    //
    //   【渲染目标】
    //   - SetRenderTarget：设置当前渲染目标
    //   - SetRandomWriteTarget：设置 UAV 随机写入目标（Compute Shader 输出）
    //
    //   【程序化绘制】
    //   - DrawProcedural / DrawProceduralIndirect：无 Mesh 绘制
    //
    //   【同步与执行】
    //   - ExecuteCommandBuffer：执行 CommandBuffer 中的渲染命令
    //   - CreateGPUFence / WaitOnGPUFence：GPU 同步栅栏
    //
    // 💡 性能提示：
    //   - DrawMeshInstancedIndirect 适合大规模植被、粒子系统
    //   - Blit 适合后处理效果链
    //   - 尽量使用 CommandBuffer 而非直接调用 Graphics 方法
    //   - Graphics.DrawMesh 不受 Camera culling 影响，容易造成过度绘制
    //
    // 📍 对应 C++ 头文件：Runtime/Graphics/GraphicsScriptBindings.h
    // ==============================================================
...
{
    // ==============================================================
    // GL — 传统即时模式 OpenGL 风格渲染接口（已不推荐使用）
    //
    // 🎯 保留用于：Legacy 后处理、调试绘制、特殊平台需求
    // ⚠️  新项目应使用 CommandBuffer 或 Graphics API 替代 GL.Begin/End
    // 📌 GL 类提供了 PushMatrix/PopMatrix、Vertex、TexCoord 等 OpenGL 式调用
    // ==============================================================
...
namespace UnityEngine
{
    ...
    // ==============================================================
    // ScalableBufferManager — 动态分辨率缩放管理器
    //
    // 🎯 用于动态分辨率（Dynamic Resolution）功能
    //   - widthScaleFactor / heightScaleFactor：当前缩放比例
    //   - ResizeBuffers：调整渲染纹理尺寸
    // 💡 配合 FrameTimingManager 使用，根据 GPU 负载自动调节渲染分辨率
    // ==============================================================
    ...
    // ==============================================================
    // FrameTiming — 帧性能计时数据结构体
    //
    // 🎯 包含一帧的完整 CPU/GPU 时间信息：
    //   - cpuFrameTime：总 CPU 帧时间
    //   - cpuMainThreadFrameTime：主线程活跃时间
    //   - cpuRenderThreadFrameTime：渲染线程活跃时间
    //   - gpuFrameTime：GPU 执行时间
    // ⚡ 用于性能分析和动态分辨率决策
    // ==============================================================
    ...
    // ==============================================================
    // FrameTimingManager — 帧计时数据采集管理器
    //
    // 🎯 提供 CaptureFrameTimings / GetLatestTimings 方法采集性能数据
    // 💡 配合 FrameTiming 结构体使用，用于动态分辨率、自适应画质等功能
    // ==============================================================
...
{
    // ==============================================================
    // GeometryUtility — 几何工具类
    //
    // 🎯 提供视锥体裁剪测试和包围盒计算
    //   - TestPlanesAABB：测试 AABB 是否在视锥体内（视锥体裁剪核心）
    //   - ExtractPlanes：从投影矩阵提取裁剪面
    //   - CalculateBounds：根据顶点数组计算包围盒
    // ==============================================================
    [NativeHeader("Runtime/Graphics/GraphicsScriptBindings.h")]
    {
...
{
    // ==============================================================
    // LightmapData — 单张光照贴图数据
    //
    // 🎯 包含光照贴图的三种纹理：
    //   - lightmapColor：RGB 颜色 + 额外的光照信息
    //   - lightmapDir：方向光照数据（用于动态物体的定向光贡献）
    //   - shadowMask：阴影遮罩（用于混合实时光照与烘焙阴影）
    // ==============================================================
    [UsedByNativeCode]
    [StructLayout(LayoutKind.Sequential)]
    ...
    // ==============================================================
    // LightmapSettings — 场景光照贴图全局设置
    //
    // 🎯 管理场景的光照贴图数据和光照探针引用
    //   - lightmaps：当前加载的光照贴图数组
    //   - lightmapsMode：光照贴图模式（单向/双向）
    //   - lightProbes：关联的光照探针数据
    // ⚡ 修改 lightmaps 属性会触发整个场景光照重新计算
    // ==============================================================
...
{
    // ==============================================================
    // LightProbes — 光照探针资产，存储烘焙的光照探针数据
    //
    // 🎯 作用：存储场景中光照探针的烘焙数据，用于动态物体的间接光照
    // 📌 工作原理：
    //   1. 编辑器中在场景放置光照探针（形成四面体网格）
    //   2. 烘焙时计算每个探针的球谐函数（SphericalHarmonicsL2）
    //   3. 运行时根据物体位置插值最近探针的光照数据
    //
    // 💡 关键方法：
    //   - Tetrahedralize / TetrahedralizeAsync：四面体重建
    //   - GetInterpolatedProbe：获取指定位置的插值光照
    //   - CalculateInterpolatedLightAndOcclusionProbes：批量计算
    //
    // ⚡ 性能提示：
    //   - 光照探针数量影响烘焙时间和运行时插值性能
    //   - 使用 LightProbesQuery（Job System）替代 GetInterpolatedProbe 可多线程查询
    // ==============================================================
...
namespace UnityEngine
{
    ...
    // ==============================================================
    // ColorGamutUtility — 色彩域工具类
    //
    // 🎯 查询色彩域的原色、白点和传输函数
    //   用于 HDR/SDR 管线中色彩空间转换
    // ==============================================================
...
{
    // ==============================================================
    // PIX — Windows 平台 GPU 性能分析器接口
    //
    // 🎯 仅在 Windows 平台 + Profiler 开启时可用
    //   提供 GPU 捕获开始/结束/检测 API，配合 PIX 工具使用
    // ==============================================================
...
{
    // ==============================================================
    // ExternalGPUProfiler — 外部 GPU 分析器接口（跨平台版 PIX）
    //
    // 🎯 提供 BeginGPUCapture / EndGPUCapture 用于外部 GPU 分析工具
    //   与 PIX 类似但支持更多平台
    // ==============================================================
    [NativeHeader("Runtime/Graphics/GraphicsScriptBindings.h")]
...
{
    // ==============================================================
    // GraphicsDeviceSettings — 图形设备同步点配置
    //
    // 🎯 控制 GPU/CPU 同步策略：
    //   - waitForPresentSyncPoint：等待呈现的同步点（帧开始/帧结束）
    //   - graphicsJobsSyncPoint：多线程渲染 Job 的同步点
    // ⚡ 这些设置影响帧延迟和 GPU 利用率，高级用户可调优
    // ==============================================================
...
{
    // ==============================================================
    // LoadStoreActionDebugModeSettings — 加载/存储操作调试模式
    //
    // 🎯 启用后可可视化渲染目标的 Load/Store 操作
    //   帮助优化 RenderPass 的 LoadAction 和 StoreAction 配置
    // ==============================================================
