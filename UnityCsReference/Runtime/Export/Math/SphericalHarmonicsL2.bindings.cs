...
// ==============================================================
// 🎯 SphericalHarmonicsL2 — 球谐函数（L2 阶，3 颜色 × 9 系数）
//
// 📌 作用：
//   存储和计算光照探头的球谐系数，用于全局光照 / 环境光照
//
// 💡 关键概念：
//   - 3 通道 (R/G/B) × 9 系数 = 27 个 float
//   - AddAmbientLight(): 添加环境光
//   - AddDirectionalLight(): 添加方向光（强度自动乘 2）
//   - Evaluate(): 在一组方向上求值，得到颜色数组
//   - Clear(): 清零所有系数
//
// 🎯 支持运算符重载：+（叠加）、*（缩放）、== / != 比较
// ⚠️ 所有系数都暴露为私有字段，通过索引器 this[rgb, coefficient] 访问
// ==============================================================
