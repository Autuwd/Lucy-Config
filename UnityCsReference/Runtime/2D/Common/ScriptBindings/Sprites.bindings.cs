...
// ==============================================================
// Sprite — Unity 2D 精灵资源核心类
//
// 🎯 核心作用：
//   Sprite 是 Unity 2D 系统的基础资源单元。
//   它定义了纹理中的一个矩形区域、Pivot（轴心点）、Border（边框）
//   以及网格信息，是 SpriteRenderer 渲染的数据源。
//
// 📌 关键属性：
//   - rect：在源纹理中的矩形区域（像素坐标）
//   - pivot：轴心点，决定旋转/缩放的中心和渲染偏移
//   - border：四边边框值（left/bottom/right/top），
//     用于九宫格切片（9-Slicing）渲染
//   - pixelsPerUnit：每单位对应的像素数，决定 Sprite 在世界空间中的大小
//   - vertices/triangles/uv：Sprite 的网格几何数据
//
// 💡 理解关键：
//   - 创建 Sprite 时的 border 参数对应四边边框：
//     Vector4(left, bottom, right, top)
//   - 当 DrawMode = Sliced 时，border 定义了九宫格的切分线
//   - packed/packingMode/packingRotation 表示 Sprite 是否被打包进 SpriteAtlas
//   - Sprite 的 extrude 属性控制网格边缘向外扩展的像素数
//
// 🏗 创建方式：
//   - Sprite.Create(texture, rect, pivot, pixelsPerUnit, extrude, meshType, border)
//   - 编辑器中通过 Import Settings 自动生成
//
// 📍 对应 C++ 头文件：
//   Runtime/Graphics/SpriteFrame.h
//   Runtime/2D/Common/SpriteDataAccess.h
// ==============================================================
...
{
    // ==============================================================
    // SpriteMeshType — Sprite 网格类型
    // ==============================================================
    // 控制 Sprite 的碰撞和渲染网格形状：
    //   - FullRect：使用矩形网格（性能好但碰撞不够精确）
    //   - Tight：使用紧密贴合的网格（碰撞精确但顶点多）
    // ==============================================================
    ...
    // ==============================================================
    // SpriteAlignment — Sprite 对齐方式（轴心点预设）
    // ==============================================================
    // 定义 Sprite 的 Pivot（轴心点）位置：
    //   - Center：中心 (0.5, 0.5)
    //   - TopLeft：左上 (0, 1) — 2D 游戏常用
    //   - BottomLeft：左下 (0, 0) — 像素游戏常用
    //   - Custom：自定义位置
    // 轴心点影响渲染偏移、旋转中心和 Inspector 中的 Position
    // ==============================================================
    ...
    // ==============================================================
    // SpritePackingMode — Sprite 打包模式
    // ==============================================================
    // SpriteAtlas 打包时的网格生成方式：
    //   - Tight：紧密打包，生成紧贴 Sprite 轮廓的网格
    //   - Rectangle：矩形打包，简单高效
    // ==============================================================
    ...
    // ==============================================================
    // SpritePackingRotation — Sprite 打包旋转选项
    // ==============================================================
    // 允许 SpriteAtlas 在打包时旋转 Sprite 以优化空间利用：
    //   - None：不旋转
    //   - FlipHorizontal/FlipVertical：水平/垂直翻转
    //   - Rotate180：旋转 180°
    //   - Any：允许任意优化旋转（位掩码 15 = 所有选项的组合）
    // ==============================================================
    ...
    // ==============================================================
    // SpriteSortPoint — Sprite 排序参考点
    // ==============================================================
    // 决定 Sprite 在排序系统中的位置：
    //   - Center：使用 Sprite 的几何中心排序（默认）
    //   - Pivot：使用 Sprite 的 Pivot 点排序
    // 当 Sprite 有不同 Pivot 时，Pivot 模式可获得更准确的前后关系
    // ==============================================================
    ...
    // Sprite — Unity 2D 精灵资源核心类
    // ==============================================================
    // 🎯 精灵是 2D 渲染的基本单元，定义了纹理区域、网格和元数据
    //
    // 📌 核心属性：
    //   rect：在源纹理中的矩形区域（像素坐标）
    //   pivot：轴心点（像素单位），影响渲染偏移和旋转中心
    //   border：九宫格边框 Vector4(left, bottom, right, top)
    //   pixelsPerUnit：每世界单位对应的像素数（C++ 端仍称 pixelsToUnits）
    //   bounds：渲染边界（世界空间）
    //   texture：关联的纹理
    //   vertices/triangles/uv：网格几何数据
    //   packed/packingMode/packingRotation：打包信息
    //   textureRect/textureRectOffset：图集中的纹理区域和偏移
    //   spriteAtlasTextureScale：图集纹理缩放
    //   associatedAlphaSplitTexture：Alpha 通道分离纹理（ETC1 压缩用）
    //
    // ⚡ BlendShape API：
    //   支持 Sprite 级别的 BlendShape（变形形状），
    //   可通过 AddBlendShape/GetBlendShapeFrame 等方法操作
    //   用于 2D 角色表情动画等场景。
    //
    // 📌 物理轮廓（Physics Outline）：
    //   用于 2D 碰撞体的轮廓定义，支持多条闭合轮廓线。
    //   可通过 OverridePhysicsOutline 运行时修改。
    // ==============================================================
