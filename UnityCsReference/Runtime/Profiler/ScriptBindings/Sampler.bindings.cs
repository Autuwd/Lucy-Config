// ====================================================================================================
// 🎯 Sampler / CustomSampler —— Profiler 采样器基类与自定义采样器
//
// 【功能定位】
//   Sampler 是旧的 Profiler 采样器基类（class），用于按名称查找 profiler 标记并获取 Recorder。
//   CustomSampler 是其子类，提供 Create / Begin / End API —— 是 Profiler.BeginSample 的现代替代。
//
// 【架构演进】
//   📌 Profiler.BeginSample(string)    —— 第一代（每帧字符串查找，有 GC）
//   📌 CustomSampler.Create(name).Begin() —— 第二代（预创建标记，减少字符串查找）
//   📌 ProfilerMarker (struct).Begin()  —— 第三代（值类型，零分配，Burst 支持）
//   ⚠️ CustomSampler 仍是 class（引用类型），ProfilerMarker 才是最终推荐方案。
//
// 【Sampler 核心】
//   💡 Sampler.Get(name)        —— 根据名称查找已注册 profiler 标记
//   💡 Sampler.GetNames(list)    —— 枚举所有可用标记名称（通过 ProfilerRecorderHandle.GetAvailable）
//   💡 GetRecorder()            —— 创建 Recorder 实例，将 Sampler + Recorder 传统 API 串联
//
// 【CustomSampler 核心】
//   💡 CustomSampler.Create(name, collectGpuData) —— 创建自定义采样器，可选 GPU 采样
//   💡 Begin() / Begin(Object) / End() —— [Conditional("ENABLE_PROFILER")]，Release 剥离
//   📌 BindingsMarshaller 内部类提供 ConvertToNative 供 IL2CPP 封送处理
//   ⚠️ static s_InvalidCustomSampler 单例标记无效实例，减少 null 检查开销
// ====================================================================================================
using System;
