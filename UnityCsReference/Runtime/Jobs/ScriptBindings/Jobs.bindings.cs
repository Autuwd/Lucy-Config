...
// =================================================================================================
// 🎯 C# Job System — 底层调度绑定层
// =================================================================================================
// Unity C# Job System 的核心调度基础设施，通过 JobsUtility 静态类暴露所有原生（C++）调度能力。
//
// 📌 核心类型
//   - JobRanges: 并行任务的索引范围，用于 ParallelFor 作业拆分工作负载
//   - ScheduleMode: Run（主线程立即执行）/ Parallel（多线程并行）/ Single（单 worker 执行）
//   - JobsUtility: 静态工具类，提供 Schedule / ScheduleParallelFor 等入口
//
// 💡 调度流程
//   1. 用户定义实现 IJob / IJobParallelFor 的结构体
//   2. JobsUtility.CreateJobReflectionData() 通过 burst 反射生成调度元数据
//   3. JobsUtility.Schedule() / ScheduleParallelFor() 将作业提交到原生 C++ JobSystem
//   4. 原生系统使用工作窃取（work stealing）算法均衡负载
//
// ⚡ NativeContainer 安全机制
//   - PatchBufferMinMaxRanges 在 Debug 模式下注入范围检查
//   - AtomicSafetyHandle 在 NativeArray/NativeList 等容器层级提供读写竞争检测
//   - 所有 IsThreadSafe = true 的 FreeFunction 保证线程安全调用
//
// 📌 JobWorkerCount / ThreadIndex
//   - JobWorkerCount: 当前活跃 worker 线程数（运行时可变）
//   - ThreadIndex: 当前作业线程索引（分布式调试/分区分配用）
//   - MaxJobThreadCount = 128: 硬编码最大线程数上限
// =================================================================================================
