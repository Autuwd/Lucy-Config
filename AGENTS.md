...
## 全局知识库（2026-09-25 收口，长期生效）
- **唯一主库**：`~/.config/opencode/学习知识库.md`（全局，跨领域知识点 + 遗忘曲线唯一权威表）。各领域目录**不再保留** `学习知识库.md` 副本
- **归档位置（例外）**：各领域笔记归档到 `E:\Lucy-Config\[领域名]\`，但 **《学习知识库.md》单独归档到 `E:\Lucy-Config\学习知识库.md`（仓库根目录，不放子目录）**
...
## 遗忘曲线复习规则（全局，2026-08-10 确立，长期生效）
- **每日复习双轨制**：每天学习开始的复习环节，除当日计划复习行外，必须检查 `~/.config/opencode/学习知识库.md` 的「遗忘曲线复习跟踪」表，把今日到期条目（下次复习日期 ≤ 今天且未标记已复习）逐一复习并更新状态
- **新内容必登记**：每天学习结束，把当天重点内容登记进遗忘曲线复习表（学习日期 | 主题 | 下次复习日期=明天），与《对话知识点记录.md》同步，不允许只记一条线
- **复习节奏**：1天 → 7天 → 16天 → 30天 一轮打完；复习通过即把"下次复习日期"推进到下一档，未通过记 `需再复习` 次日重测
- **状态约定**：`已复习(mm-dd)` / `需再复习` / `红标`；红标项次日复习优先重测（与各项目计划红标机制合并处理）
- **登记粒度**：按"天×板块"登记一条，主题栏写明可检索关键词

## Git 备份规则（全局）
- **学习内容备份**：无论在哪个目录学习，学习笔记都保存在当前目录
- **统一归档**：学习结束后，将笔记复制到 `E:\Lucy-Config\[领域名]\` 下；**《学习知识库.md》例外 → `E:\Lucy-Config\学习知识库.md`（仓库根目录）**
- **每日备份**：执行 `cd E:\Lucy-Config && git add . && git commit -m "[领域]：[主题]" && git push` 备份到 GitHub
- **备份时机**：学习结束时主动提醒用户是否要备份
- **编码安全（2026-08-10 确立，长期生效）**：所有归档 md 文件统一 **UTF-8 无 BOM**；归档前必须校验源文件编码（可 UTF-8 解码、不含 U+FFFD/锟斤拷），**禁止**以 ANSI/GBK 编码另存覆盖、禁止把乱码文件复制进仓库；归档后对比源与目标**字节数一致**；发现损坏先存档损坏稿到 temp 再修复，禁止直接推送损坏文件

## 学习 md 纯文字规则（全局，2026-09-19 确立，长期生效）
> 背景教训：2026-08-07 一次 GBK 保存中 emoji 无对应编码、被写成 `?`，导致《学习知识库.md》约 234 处 emoji 永久丢失（不可逆）；而现行编码校验只查 U+FFFD / 锟斤拷，`?` 是合法 ASCII，完全检测不到。

- **所有学习 md 一律禁用 emoji**（✅ ❌ 🔴 🟡 🟢 ⭐ ⚠ 💡 📖 🔗 📝 📌 🎯 🔧 🧩 📋 ⏳ 等全部 emoji / 符号表情）
- **状态一律用纯文字**：`待开始` / `进行中` / `已完成` / `未完成` / `已复习` / `需再复习` / `红标`
- **强调用纯文字方括号标记**：`【重点】` `【注意】` `【补充】` `【工具】` `【引用】` `【深入解析】` `【对照】` `【思考】`
- **适用范围**：学习计划 / 面试题库 / 知识库 / 对话知识点记录 / Day 笔记 / 书籍笔记 / 归档 md 等全部学习 md；git commit message 不受本条限制（但建议同样纯文字）
- **例外**：AGENTS.md 规则文件本身为说明用途可列出 emoji 示例，不受本条限制
- **新增校验（与编码校验一起跑）**：学习 md 不应出现 `? ` / `?? ` 形式的状态标记；若发现即为异常（emoji 丢失或 GBK 残留），须先修复再备份
